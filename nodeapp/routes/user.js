...
function validate_name(req, res, next) {
  const vietnameseRegex =
    /^[a-vxyỳọáầảấờễàạằệếýộậốũứĩõúữịỗìềểẩớặòùồợãụủíỹắẫựỉỏừỷởóéửỵẳẹèẽổẵẻỡơôưăêâđ]+$/i;
  let user = req.body;
...
user_router.post("/", validate_age, validate_name, (req, res) => {
  ...
  connection.query(
    ...
    [user.fullname, parseInt(user.age), parseInt(user.gender)],
...
user_router.put("/:id", validate_age, validate_name, (req, res) => {
  ...
  connection.query(
    ...
    [user.fullname, user.age, user.gender, parseInt(req.params.id)],
    function (err, result, fields) {
      if (err) throw err;
    }
  );
